...
def distinctive_hi_mr(text: str) -> tuple[int, int]:
    """Count Hindi-only and Marathi-only dictionary words. Shared words are ignored."""
    hindi = LANGUAGE_WORDS["Hindi"]["native"]
    marathi = LANGUAGE_WORDS["Marathi"]["native"]
    tokens = set(_tokenize(text))
    return len(tokens & (hindi - marathi)), len(tokens & (marathi - hindi))


def marathi_overrides_hindi(parts: list[tuple[str, str]]) -> bool:
    """Customer Marathi words beat a Hindi agent greeting.

    Uses the customer channel when it exists. A Hindi-only call has no
    Marathi-only words, so it stays Hindi.
    """
    customer = " ".join(
        text for label, text in parts if str(label).startswith("customer")
    )
    source = customer or " ".join(text for _label, text in parts)
    hindi_only, marathi_only = distinctive_hi_mr(source)
    return marathi_only >= 2 and marathi_only > hindi_only


def bengali_blocked_by_devanagari(text: str) -> bool:
    """Devanagari speech must not be labeled Bengali from a weak roman score."""
    devanagari = len(SCRIPT_RES["devanagari"].findall(text or ""))
    bengali = len(SCRIPT_RES["bengali"].findall(text or ""))
    return devanagari >= 8 and bengali < 4


def kannada_blocked_by_devanagari(text: str) -> bool:
    """Devanagari speech must not be labeled Kannada from a weak roman score."""
    devanagari = len(SCRIPT_RES["devanagari"].findall(text or ""))
    kannada = len(SCRIPT_RES["kannada"].findall(text or ""))
    return devanagari >= 8 and kannada < 4
...
def detect_language_wordmatch(
    ...
    try:
        ...
        # Hindi/Bengali forced-decode check.
        # A close score is a tiebreak. A Bengali winner is always re-checked,
        # because a high Bengali dictionary score can come from Whisper writing
        # the wrong script for Hindi speech.
        ...
        if (
            ...
            try:
                ...
                _log("hi/bn word tiebreak: hi_hits=%d bn_hits=%d | hi='%.60s' bn='%.60s'",
                     hi_hits, bn_hits, hi_text, bn_text)
                if winner == "Bengali" and hi_hits >= 2 and hi_hits > bn_hits:
                    winner = "Hindi"
                    _log("Bengali rejected — forced Hindi words win")
                elif winner != "Bengali" and hi_hits != bn_hits:
                    winner = "Hindi" if hi_hits > bn_hits else "Bengali"
            except Exception as exc:
                _log("hi/bn tiebreak failed (%s) — keeping %s", exc, winner)

        parts = [(label, text) for label, _path, text in transcripts]
        joined = " ".join(text for _label, text in parts)
        if winner == "Hindi" and marathi_overrides_hindi(parts):
            _log("Marathi distinctive words override Hindi")
            winner = "Marathi"
        if winner == "Bengali" and bengali_blocked_by_devanagari(joined):
            _log("Bengali blocked — transcript is Devanagari")
            winner = "Hindi"
        if winner == "Kannada" and kannada_blocked_by_devanagari(joined):
            _log("Kannada blocked — transcript is Devanagari")
            winner = "Hindi"
        elif winner == "Kannada" and not forced_checked:
            # Whisper auto-decode sometimes writes Kannada script for Hindi
            # telephone speech. Keep Kannada only when forced Kannada produces
            # more real Kannada words than forced Hindi.
            best_path = transcripts[0][1] if transcripts else snippets[0][1]
            try:
                hi_text = transcribe_forced(best_path, "hi")
                kn_text = transcribe_forced(best_path, "kn")
                hi_hits = _native_word_hits(hi_text, "Hindi")
                kn_hits = _native_word_hits(kn_text, "Kannada")
                _log(
                    "Kannada cross-check: hi_hits=%d kn_hits=%d | hi='%.60s' kn='%.60s'",
                    hi_hits, kn_hits, hi_text, kn_text,
                )
                if hi_hits >= 2 and hi_hits > kn_hits:
                    winner = "Hindi"
                    _log("Kannada rejected — forced Hindi words win")
            except Exception as exc:
                _log("Kannada cross-check failed (%s) — keeping Kannada", exc)
