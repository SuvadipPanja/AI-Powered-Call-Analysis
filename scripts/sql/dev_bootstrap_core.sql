...
BEGIN
    CREATE TABLE dbo.AudioUploads (
        ...
        SelectedAgent    NVARCHAR(200) NULL,
...
BEGIN
    CREATE TABLE dbo.AI_Processing_Result (
        ...
        AudioWPM         FLOAT NULL,
        AudioDiarization NVARCHAR(MAX) NULL,
        ToneAnalysis     NVARCHAR(MAX) NULL,
        AIScoring        NVARCHAR(50) NULL,
        ManualScoring    NVARCHAR(50) NULL,
...
/* Patch pre-existing dev databases created by an earlier version of this
 * script (or by partial runtime provisioning) so read-path queries in the
 * backend (dashboard metrics, reports, result page) never hit a missing
 * column. Each ALTER is guarded and therefore idempotent. */
DECLARE @patch TABLE (tbl SYSNAME, col SYSNAME, def NVARCHAR(200));
INSERT INTO @patch (tbl, col, def) VALUES
    ('AudioUploads',        'SelectedAgent',    'NVARCHAR(200) NULL'),
    ('AudioUploads',        'ProcessStatus',    'NVARCHAR(50) NULL'),
    ('AI_Processing_Result','TranslateOutput',  'NVARCHAR(MAX) NULL'),
    ('AI_Processing_Result','AudioLanguage',    'NVARCHAR(50) NULL'),
    ('AI_Processing_Result','AudioDuration',    'NVARCHAR(50) NULL'),
    ('AI_Processing_Result','AudioWPM',         'FLOAT NULL'),
    ('AI_Processing_Result','AudioDiarization', 'NVARCHAR(MAX) NULL'),
    ('AI_Processing_Result','ToneAnalysis',     'NVARCHAR(MAX) NULL'),
    ('AI_Processing_Result','AIScoring',        'NVARCHAR(50) NULL'),
    ('AI_Processing_Result','ManualScoring',    'NVARCHAR(50) NULL'),
    ('AI_Processing_Result','TranslationModel', 'NVARCHAR(100) NULL'),
    ('AI_Processing_Result','ScoringModel',     'NVARCHAR(100) NULL'),
    ('AI_Processing_Result','ASREngine',        'NVARCHAR(200) NULL');

DECLARE @tbl SYSNAME, @col SYSNAME, @def NVARCHAR(200), @sql NVARCHAR(400);
DECLARE patch_cur CURSOR LOCAL FAST_FORWARD FOR SELECT tbl, col, def FROM @patch;
OPEN patch_cur;
FETCH NEXT FROM patch_cur INTO @tbl, @col, @def;
WHILE @@FETCH_STATUS = 0
BEGIN
    IF OBJECT_ID('dbo.' + @tbl, 'U') IS NOT NULL
       AND NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = @tbl AND COLUMN_NAME = @col)
    BEGIN
        SET @sql = 'ALTER TABLE dbo.[' + @tbl + '] ADD [' + @col + '] ' + @def;
        EXEC sp_executesql @sql;
        PRINT 'Patched column: ' + @tbl + '.' + @col;
    END
    FETCH NEXT FROM patch_cur INTO @tbl, @col, @def;
END
CLOSE patch_cur;
DEALLOCATE patch_cur;
GO
